  ...
  <div class="relative py-16 overflow-hidden bg-white">
    ...
    <div class="relative px-4 sm:px-6 lg:px-8">
      <div class="mx-auto text-lg max-w-prose">
        <h1>
          <span class="block text-base font-semibold tracking-wide text-center text-blue-600 uppercase">Preparation</span>
          <span class="block mt-2 text-3xl font-extrabold leading-8 tracking-tight text-center text-gray-900 sm:text-4xl">Create Folders to Backup Your Data</span>
        </h1>
      </div>
      <nuxt-content class="pt-4 pb-12 mx-auto prose lg:prose-lg xl:prose-xl sm:pb-24 sm:pt-8" :document="article" />
    </div>
  </div>
</template>
...
export default {
  async asyncData({ $content }) {
    const article = await $content('prep').fetch()
